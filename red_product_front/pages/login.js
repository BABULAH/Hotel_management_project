...
const Login = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
        ...
        const { token, user } = response.data; // Récupérer aussi les données de l'utilisateur

        if (token) {
            if (rememberMe) {
                localStorage.setItem('token', token); // Stocker le token
                localStorage.setItem('user', JSON.stringify(user)); // Stocker les données de l'utilisateur
            } else {
                sessionStorage.setItem('token', token); // Stocker temporairement le token pour la session active
                sessionStorage.setItem('user', JSON.stringify(user)); // Stocker temporairement les données de l'utilisateur
            }
